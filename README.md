...
### 📂 批量 URL 扫描（新功能）
支持从文件批量导入 URL 进行扫描，每个 URL 独立扫描，完整检测：
- **独立扫描模式**: 不使用 SQLMap 的批量模式，而是逐个 URL 独立扫描，确保每个目标都能完整检测，不会因发现漏洞而跳过其他 URL
- **实时进度显示**: 状态栏显示当前扫描进度 `[1/3]`, `[2/3]` 等
- **批量结果展示**: 专门的批量结果标签页，以表格形式展示所有 URL 的扫描结果
- **结果筛选**: 可按"全部"、"有漏洞"、"安全"、"已跳过"等状态筛选结果
- **一键导出**: 支持导出批量结果为 **CSV** 或 **JSON** 格式
- **复制到剪贴板**: 快速复制结果分享

![批量扫描结果](assets/批量结果.png)
...
- 🟡 **进阶方案推荐**：中等风险，增加请求复杂度绕过防护
- 🔴 **激进方案推荐**：高效但有风险，需用户确认后使用
- 🛡️ **WAF 识别指导**：自动识别常见 WAF（宝塔/安全狗/CloudFlare 等）
- 📦 **多种绕过策略**：Tamper脚本、请求头伪装、编码绕过、延时隐蔽、注入技术调整
...
告别手动下载更新！程序启动时会自动检查是否有新版本，发现更新时弹窗提醒。

**🚀 启动自动检查：**
- 程序启动 3 秒后自动检查更新
- 发现新版本时弹出提示框，询问是否更新
- 无新版本或网络异常时静默处理，不打扰用户

**检查更新：**

![check_update](images/check_update.png)

点击 **帮助 → 🔄 检查更新**，程序会从 GitHub 获取最新版本信息，如有更新可一键下载安装。

**下载/更新 SQLMap：**

![download_sqlmap](images/download_sqlmap.png)

点击 **帮助 → 📥 下载/更新 SQLMap**，自动从 SQLMap 官方仓库下载最新版本并解压到程序目录，无需手动配置。

**核心特性：**
- 📢 **启动自动检查**：程序启动时后台静默检查，有新版本才弹窗提醒
- 🌐 **一键更新**：从 GitHub 自动下载最新版本
- 📦 **自动解压**：下载完成后自动解压覆盖旧版本
- 💾 **保护配置**：更新时自动保留用户配置和历史记录
- 📊 **进度显示**：下载进度实时显示
...
启动程序后，点击 **帮助 → 📥 下载/更新 SQLMap**，自动下载最新版 SQLMap。
